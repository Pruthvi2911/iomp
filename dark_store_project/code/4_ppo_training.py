...
# Training parameters
TOTAL_TIMESTEPS = 500000  # Total training steps (enough to guarantee mastery over 3 items)
LEARNING_RATE = 3e-4
...
class ProgressCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.locals['dones'][0]:
            ...
            # Check if successful using the actual info dictionary from true episode completion
            info = self.locals['infos'][0]
            success = info.get('items_picked', 0) == info.get('total_items', 1)
...
try:
    # Create environment
    env = DarkStoreEnv(LAYOUT_FILE, max_steps=350, max_items_per_order=3)
...
model = PPO(
    ...
    policy_kwargs=dict(net_arch=[256, 256]),  # Larger neural network brain!
    verbose=1
)
...
for i in range(n_eval_episodes):
    ...
    while not done:
        # MUST BE FALSE for grid navigation! Deterministic=True will get the agent stuck in an infinite movement loop against a wall!
